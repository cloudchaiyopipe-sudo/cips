            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        console.warn(
                            '⚠️ Cesium World Terrain not available:',
                            worldTerrainError?.message || worldTerrainError
                        );
                        ...
                        try {
                            ...
                            console.warn(
                                '⚠️ ArcGIS Terrain not available:',
                                arcGisError?.message || arcGisError
                            );

                            // ลองใช้ Google Terrain (ใช้ Google Maps API)
                            try {
                                console.log(
                                    '🔄 Attempting to use Google Terrain via EllipsoidTerrainProvider with high exaggeration...'
                                );
                                ...
                                terrainProviderName =
                                    'EllipsoidTerrainProvider (no elevation data)';
                                console.warn(
                                    '⚠️ Using EllipsoidTerrainProvider (no real elevation data)'
                                );
                            } catch (ellipsoidError: any) {
                                console.error(
                                    '❌ Failed to set any terrain provider:',
                                    ellipsoidError
                                );
                            }
                        }
                    ...
                    if (terrainProviderSet) {
                        ...
                        console.log(
                            '🏔️ Terrain exaggeration set to 1.0 (natural height with real terrain provider)'
                        );
                    } else {
                        ...
                        console.warn(
                            '⚠️ No real terrain provider, using natural exaggeration:',
                            viewer.scene.globe.terrainExaggeration
                        );
                        console.warn(
                            '⚠️ Note: EllipsoidTerrainProvider has no elevation data, terrain will still appear flat'
                        );
                    }
                    ...
                    setTimeout(() => {
                        ...
                        console.log(
                            ...
                            currentTerrain?.constructor?.name || 'Unknown'
                        );
                ...
                try {
                    if (use3DTiles) {
                        ...
                        console.log(
                            '🔄 Loading 3D Photorealistic Tiles mode (with terrain data)...'
                        );
                        ...
                        try {
                            ...
                            console.log(
                                '✅ Base imagery layer added (will show in areas without 3D Tiles coverage)'
                            );
                        } catch (baseError) {
                        ...
                        console.log(
                            '✅ Google Photorealistic 3D Tiles will overlay on top (where available)'
                        );

                        // โหลด 3D Tiles - สำคัญ: 3D Tiles มี terrain/elevation data อยู่แล้ว
                        console.log(
                            '🏔️ Loading Google Photorealistic 3D Tiles (includes terrain data)...'
                        );
                        ...
                        if (tileset.readyPromise) {
                            ...
                            console.log(
                                '✅ 3D Tiles added (readyPromise not available in this Cesium version)'
                            );
                        }
                        ...
                        try {
                            console.log(
                                '🔄 Attempting to add ArcGIS Terrain for globe elevation...'
                            );
                            const arcGisTerrain = new Cesium.ArcGisMapServerTerrainProvider({
                                url: 'https://elevation3d.arcgis.com/arcgis/rest/services/WorldElevation3D/Terrain3D/ImageServer',
                            });
                            ...
                            console.warn(
                                '⚠️ 3D Tiles should still show terrain, but globe will be flat'
                            );
                        }

                        console.log('✅ 3D Tiles mode loaded with performance optimizations');
                        console.log(
                            '✅ Google Photorealistic 3D Tiles includes terrain/elevation data in tiles'
                        );
                        console.log(
                            '🏔️ Terrain exaggeration:',
                            viewer.scene.globe.terrainExaggeration,
                            '(1.0 = natural height)'
                        );
                        ...
                        console.log(
                            '🔄 Loading Hybrid Imagery mode (with full labels visibility)...'
                        );
                        console.log(
                            '⚠️ Warning: Hybrid mode requires real terrain provider for elevation data'
                        );
                        ...
                        if (!terrainProviderInfoRef.set) {
                            ...
                            console.error(
                                '❌ Terrain will appear FLAT because EllipsoidTerrainProvider has no elevation data'
                            );
                            console.error(
                                '💡 Solution: Enable use3DTiles=true to use Google Photorealistic 3D Tiles (has terrain data)'
                            );
                            console.error(
                                '💡 Or: Get Cesium Ion access token for Cesium World Terrain'
                            );
                        }
                    ...
                    setTimeout(() => {
                        ...
                        for (let i = 0; i < viewer.imageryLayers.length; i++) {
                            ...
                            if (layer) {
                                console.log(
                                    `  Layer ${i}: ${layer.imageryProvider?.credit || 'Unknown'}, alpha: ${layer.alpha}`
                                );
                            }
                        }
                    }, 2000);
                ...
                if (mainArea && mainArea.length > 0) {
                    setTimeout(() => {
                        try {
                            const positions = mainArea.map((coord) =>
