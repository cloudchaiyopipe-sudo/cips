...
function findHeadLoss(
    ...
    const flowRates = Object.keys(pipeTable.data).map(Number).sort((a, b) => a - b);
...
function getAdjustmentFactor(outlets: number): number {
    ...
    const outletKeys = Object.keys(factors).map(Number).sort((a, b) => a - b);
...
function calculateOptimalPipeSize(
    ...
    const selectedTable = irrigationData.pipeHeadLossTables[pipeTypeKey as keyof typeof irrigationData.pipeHeadLossTables];
    ...
    let bestSize: { sizeMM: number; sizeOriginal: number; sizeInch: string; index: number; hf: number; pressureLoss: number } | null = null;
...
function enforcePipeSizeHierarchy(
    result: PipeTypeRecommendations
...
function calculatePipeSizeForType(
    ...
    const peResult = calculateOptimalPipeSize(flowRate, longestLength, outlets, peTableKey, peTableName, maxPressureLoss);
    if (peResult && peResult.sizeMM > 0) {
        result.pe = peResult;
    }
    
    // คำนวณ PVC (ใช้ความยาวท่อที่ยาวที่สุด)
    const pvcResult = calculateOptimalPipeSize(flowRate, longestLength, outlets, pvcTableKey, pvcTableName, maxPressureLoss);
...
export const calculatePipeSize = (
    ...
    return typeResult.pe || typeResult.pvc || {
        sizeMM: 0,
        sizeInch: '0"',
...
export const calculatePipeRecommendations = (
    ...
    return {
        main: pipeTypeRecs.main.pe || pipeTypeRecs.main.pvc || {
            sizeMM: 0,
            sizeInch: '0"',
            reason: 'ไม่สามารถคำนวณขนาดท่อได้',
            pressureLoss: 0
        },
        subMain: pipeTypeRecs.subMain.pe || pipeTypeRecs.subMain.pvc || {
            sizeMM: 0,
            sizeInch: '0"',
            reason: 'ไม่สามารถคำนวณขนาดท่อได้',
            pressureLoss: 0
        },
        lateral: pipeTypeRecs.lateral.pe || pipeTypeRecs.lateral.pvc || {
            sizeMM: 0,
            sizeInch: '0"',
            reason: 'ไม่สามารถคำนวณขนาดท่อได้',
            pressureLoss: 0
...
export const calculatePipeRecommendationsWithTypes = (
    ...
    const lateralLongestFlowRate = zoneData?.lateralLongestFlowRate || (lateralFlowRate * lateralOutlets);
    ...
    // หรือใช้ค่าที่ส่งมา
    const subMainLongestFlowRate = zoneData?.subMainLongestFlowRate || (lateralLongestFlowRate * subMainOutlets);
    ...
    // หรือใช้ค่าที่ส่งมา
    const mainLongestFlowRate = zoneData?.mainLongestFlowRate || (subMainLongestFlowRate * mainOutlets);
    ...
    if (lateralLossForCalculation > subMainLossSelfForCalculation && subMainLossSelfForCalculation > 0) {
    ...
    if (totalLoss > totalLossMax) {
        console.warn(`⚠️ Total pressure loss (worst-case: ${totalLoss.toFixed(2)}m) exceeds maximum allowed (${totalLossMax.toFixed(2)}m = 20% of ${totalHead.toFixed(2)}m head)`);
        console.warn(`   - Lateral loss (worst-case): ${lateralLossWorstCase.toFixed(2)}m`);
        console.warn(`   - SubMain loss (worst-case, รวม lateral): ${subMainLossWorstCase.toFixed(2)}m`);
        console.warn(`   - Main loss (worst-case, รวม lateral + subMain): ${mainLoss.toFixed(2)}m`);
        console.warn(`   💡 Suggestion: Consider increasing water pressure or using larger pipe sizes.`);
    } else {
        console.log(`✅ Total pressure loss (worst-case) is within limit: ${totalLoss.toFixed(2)}m / ${totalLossMax.toFixed(2)}m (20% of ${totalHead.toFixed(2)}m head)`);
        console.log(`   - Lateral loss (worst-case): ${lateralLossWorstCase.toFixed(2)}m`);
        console.log(`   - SubMain loss (worst-case, รวม lateral): ${subMainLossWorstCase.toFixed(2)}m`);
