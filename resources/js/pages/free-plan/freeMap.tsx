...
function FreeMap() {
        ...
        setPlantPoints(prevPlantPoints => {
            ...
            const updated = prevPlantPoints.filter(point => {
                if (!geometry?.spherical) {
                    ...
                    const close = Math.abs(point.position.lat - position.lat) <= degTol && Math.abs(point.position.lng - position.lng) <= degTol;
                    if (close && point.marker) {
                        (point.marker as { setMap: (map: unknown) => void }).setMap(null);
                        console.log('❌ Removed plant point (fallback method):', point.position);
                    }
                    return !close;
                }
                ...
                if (overlapped && point.marker) {
                    (point.marker as { setMap: (map: unknown) => void }).setMap(null);
                    console.log(`❌ Removed plant point at distance ${dist.toFixed(2)}m:`, point.position);
                }
                return !overlapped;
            });
    ...
    useEffect(() => {
        ...
        if (savedShapes) {
            try {
                ...
                setDrawnShapes(shapes.map((shape: { type: string; id: number; data: unknown }) => ({ ...shape, overlay: null })));
            } catch (error) {
        ...
        if (savedWaterSources) {
            try {
                ...
                setWaterSources(waterSources.map((ws: { id: number; position: { lat: number; lng: number } }) => ({ ...ws, marker: null })));
        ...
        if (savedPumps) {
            try {
                ...
                setPumps(pumps.map((pump: { id: number; position: { lat: number; lng: number } }) => ({ ...pump, marker: null })));
            } catch (error) {
        ...
        if (savedPlantPoints) {
            try {
                ...
                setPlantPoints(plantPoints.map((point: { id: number; position: { lat: number; lng: number } }) => ({ ...point, marker: null })));
            } catch (error) {
        ...
        if (savedMainPipes) {
            try {
                ...
                setMainPipes(mainPipes.map((pipe: { 
                    id: number; 
                    fromPump: { lat: number; lng: number }; 
                    toZoneCenter: { lat: number; lng: number };
                    zoneId: number;
        ...
        if (savedSubMainPipes) {
            try {
                ...
                setSubMainPipes(subMainPipes.map((pipe: { 
                    id: number; 
                    path: Array<{ lat: number; lng: number }>; 
                    zoneId: number 
                }) => ({ 
        ...
        if (savedLateralPipes) {
            try {
                ...
                setLateralPipes(lateralPipes.map((pipe: { 
                    id: number; 
                    path: Array<{ lat: number; lng: number }>; 
                    zoneId: number 
                }) => ({ 
    ...
    const createPlantPoints = useCallback((map: google.maps.Map, shapeData: { bounds?: { north: number; south: number; east: number; west: number }; center?: { lat: number; lng: number }; radius?: number; path?: Array<{ lat: number; lng: number }> }, shapeType: string) => {
        ...
        const plantPoints: Array<{ id: number; position: { lat: number; lng: number }; marker: unknown }> = [];
        let pointId = 1;
    ...
    const getDistanceFromLatLonInMeters = (point1: { lat: number; lng: number }, point2: { lat: number; lng: number }) => {
    ...
    useEffect(() => {
        ...
        const loadGoogleMaps = () => {
            ...
            script.onload = () => {
                ...
                setTimeout(() => {
                    if (isMounted && window.google && window.google.maps && window.google.maps.drawing) {
                        setApiLoading(false);
                        setMapLoaded(true);
                        initializeMap();
                    } else {
        ...
        const initializeMap = () => {
            ...
            if (savedView) {
                try {
                    const parsed = JSON.parse(savedView) as { zoom: number; center: { lat: number; lng: number } };
                    if (typeof parsed.zoom === 'number' && parsed.center?.lat && parsed.center?.lng) {
    ...
    setSearchValue('');
            ...
            const initializeDrawingManager = () => {
                try {
                    ...
                    if (!window.google?.maps?.drawing?.DrawingManager) {
                        console.warn('Google Maps drawing library not ready yet. Scheduling retry...');
                        setTimeout(() => {
            ...
            if (zones.length > 0) {
                zones.forEach(zone => {
                    if (zone.bounds && !zone.overlay) {
                        ...
                        if (zone.center) {
                            centerMarker = createZoneCenterMarker(map, zone.center, zone.name, zone.color);
                        }
            ...
            if (plantPoints.length > 0) {
                ...
                plantPoints.forEach(point => {
                    if (point.position && !point.marker) {
                        ...
                        setPlantPoints(prev => prev.map(p => 
                            p.id === point.id ? { ...p, marker } : p
                        ));
                    }
                });
            }
            ...
            if (mainPipes.length > 0) {
                mainPipes.forEach(pipe => {
                    if (pipe.fromPump && pipe.toZoneCenter && !pipe.overlay) {
                        ...
                        polyline.setMap(map);
                        setMainPipes(prev => prev.map(p => 
                            p.id === pipe.id ? { ...p, overlay: polyline } : p
                        ));
                    }
                });
            }
            ...
            if (subMainPipes.length > 0) {
                subMainPipes.forEach(pipe => {
                    if (pipe.path && pipe.path.length > 0 && !pipe.overlay) {
                        ...
                        polyline.setMap(map);
                        setSubMainPipes(prev => prev.map(p => 
                            p.id === pipe.id ? { ...p, overlay: polyline } : p
                        ));
                    }
                });
            ...
            if (lateralPipes.length > 0) {
                lateralPipes.forEach(pipe => {
                    if (pipe.path && pipe.path.length > 0 && !pipe.overlay) {
                        ...
                        polyline.setMap(map);
                        setLateralPipes(prev => prev.map(p => 
                            p.id === pipe.id ? { ...p, overlay: polyline } : p
                        ));
                    }
                });
            ...
            if (drawnShapes.length > 0) {
                drawnShapes.forEach(shape => {
                    if (shape.data) {
                        ...
                        if (shape.type === window.google.maps.drawing.OverlayType.POLYGON && shape.data.path) {
                            overlay = new window.google.maps.Polygon({
                        ...
                        } else if (shape.type === window.google.maps.drawing.OverlayType.RECTANGLE && shape.data.bounds) {
                            overlay = new window.google.maps.Rectangle({
                                bounds: new window.google.maps.LatLngBounds(
                        ...
                        if (overlay) {
                            setDrawnShapes(prev => prev.map(s => 
                                s.id === shape.id ? { ...s, overlay } : s
                            ));
                            
                            // Add click listener to the overlay for water source placement
                            window.google.maps.event.addListener(overlay, 'click', (event: { latLng: { lat: () => number; lng: () => number } }) => {
                                ...
                                if (actualCurrentStep === 1) {
                                    ...
                                    const waterMarker = createWaterSourceMarkerSimple({ lat: clickedLat, lng: clickedLng }, map as google.maps.Map);
                                    ...
                                    if (waterMarker) {
                                        ...
                                        // Save to localStorage
                                        const waterSourcesForStorage = [...waterSources, newWaterSource].map(ws => ({
                                            id: ws.id,
                                            position: ws.position
                                        }));
                                        localStorage.setItem('waterSources', JSON.stringify(waterSourcesForStorage));
                                        
                                        console.log('✅ Water source placed successfully:', newWaterSource);
                                        console.log('💾 Saved to localStorage:', waterSourcesForStorage);
                                        ...
                                        console.log('Auto-advanced to Place Pump step after placing water source');
                                        ...
                                        const placementPoints = createPumpPlacementPoints({ lat: clickedLat, lng: clickedLng }, map as google.maps.Map);
            ...
            window.google.maps.event.addListener(map, 'click', (event: { latLng: { lat: () => number; lng: () => number } }) => {
                const clickedLat = event.latLng.lat();
                const clickedLng = event.latLng.lng();
                    ...
                    drawnShapes.forEach((shape) => {
                        if (shape.data) {
                            ...
                            if (shape.type === window.google.maps.drawing.OverlayType.RECTANGLE && shape.data.bounds) {
                                ...
                                isInside = clickedLat >= bounds.south && 
                                ...
                                for (let i = 0, j = path.length - 1; i < path.length; j = i++) {
                                    if (((path[i].lat > clickedLat) !== (path[j].lat > clickedLat)) &&
                                        (clickedLng < (path[j].lng - path[i].lng) * (clickedLat - path[i].lat) / (path[j].lat - path[i].lat) + path[i].lng)) {
                                        inside = !inside;
                                    }
                                }
                                isInside = inside;
            ...
            window.google.maps.event.addListener(drawingManagerRef.current, 'overlaycomplete', (event: { overlay: unknown; type: string }) => {
                ...
                if (type === window.google.maps.drawing.OverlayType.POLYGON) {
                    const polygon = overlay as { getPath: () => { getArray: () => Array<{ lat: () => number; lng: () => number }> } };
                    ...
                    const rectangle = overlay as { getBounds: () => { getNorthEast: () => { lat: () => number; lng: () => number }; getSouthWest: () => { lat: () => number; lng: () => number } } };
                    ...
                    const circle = overlay as { getCenter: () => { lat: () => number; lng: () => number }; getRadius: () => number };
                    overlayData = {
                        center: {
                ...
                if (drawingManagerRef.current) {
                    (drawingManagerRef.current as { setDrawingMode: (mode: unknown) => void }).setDrawingMode(null);
                }
    ...
    }, [mapInitialized, completedSteps, drawnShapes, currentStep, waterSources, pumps, zones, plantPoints, mainPipes, subMainPipes, lateralPipes, createPlantPoints]); // eslint-disable-line react-hooks/exhaustive-deps
        ...
        if (zones.length > 0) {
            zones.forEach(zone => {
                if (!zone.overlay && zone.bounds) {
                    ...
                    setZones(prev => prev.map(z => z.id === zone.id ? { ...z, overlay, centerMarker } : z));
                }
            });
        }
        ...
        if (plantPoints.length > 0) {
            ...
            plantPoints.forEach(point => {
                if (!point.marker) {
                    ...
                    setPlantPoints(prev => prev.map(p => p.id === point.id ? { ...p, marker } : p));
                }
            });
        }
        ...
        if (mainPipes.length > 0) {
            mainPipes.forEach(pipe => {
                if (!pipe.overlay) {
                    ...
                    polyline.setMap(map);
                    setMainPipes(prev => prev.map(p => p.id === pipe.id ? { ...p, overlay: polyline } : p));
                }
            });
        }
        ...
        if (subMainPipes.length > 0) {
            subMainPipes.forEach(pipe => {
                if (!pipe.overlay) {
                    ...
                    polyline.setMap(map);
                    setSubMainPipes(prev => prev.map(p => p.id === pipe.id ? { ...p, overlay: polyline } : p));
                }
            });
        }
        ...
        if (lateralPipes.length > 0) {
            lateralPipes.forEach(pipe => {
                if (!pipe.overlay) {
                    ...
                    polyline.setMap(map);
                    setLateralPipes(prev => prev.map(p => p.id === pipe.id ? { ...p, overlay: polyline } : p));
                }
            });
        }
        ...
        if (drawnShapes.length > 0) {
            drawnShapes.forEach(shape => {
                if (!shape.overlay && shape.data) {
                    let overlay: google.maps.Polygon | google.maps.Rectangle | google.maps.Circle | null = null;
                    if (shape.type === window.google.maps.drawing.OverlayType.POLYGON && shape.data.path) {
                        overlay = new window.google.maps.Polygon({
                            paths: shape.data.path,
                            fillColor: '#10b981',
                    ...
                    if (overlay) {
                        overlay.setMap(map);
                        setDrawnShapes(prev => prev.map(s => s.id === shape.id ? { ...s, overlay } : s));
                    }
                }
            });
        }
    ...
    const handleReset = () => {
        ...
        if (drawingManagerRef.current) {
            (drawingManagerRef.current as { setDrawingMode: (mode: unknown) => void }).setDrawingMode(null);
            (drawingManagerRef.current as { setOptions: (options: { drawingControl: boolean }) => void }).setOptions({ drawingControl: false });
        }
    ...
    const handleDrawAreaClick = () => {
        ...
        if (isDrawingMode) {
            ...
            // Enter drawing mode
            if (drawingManagerRef.current) {
                (drawingManagerRef.current as { setOptions: (options: { drawingControl: boolean }) => void }).setOptions({ drawingControl: true });
            }
    ...
    const createWaterSourceMarkerSimple = (position: { lat: number; lng: number }, map: google.maps.Map) => {
        ...
        if (waterSources.length > 0) {
            waterSources.forEach(waterSource => {
                if (waterSource.position && !waterSource.marker) {
                    ...
                    setWaterSources(prev => prev.map(ws => 
                        ws.id === waterSource.id ? { ...ws, marker } : ws
                    ));
                }
            });
        }
    ...
    }, [currentStep, completedSteps, mapInitialized, drawnShapes.length, waterSources, pumps.length, pumpPlacementPoints, createPumpPlacementPoints]);
    ...
    */
    ...
    const performKMeansClustering = (plants: PlantLocation[], k: number): PlantLocation[][] => {
        ...
        console.log(`🎯 Starting Balanced K-means clustering for ${plants.length} plants into ${k} zones`);
        ...
        console.log(`📊 Target size per zone: ${targetSize} plants (${remainder} zones will have ${targetSize + 1})`);
        ...
        while (iteration < maxIterations) {
            ...
            const plantDistances: Array<{ plant: PlantLocation; distances: number[] }> = plants.map(plant => ({
                plant,
                distances: centroids.map(centroid => calculateDistance(plant.position, centroid))
            }));
    ...
    */
    ...
    const generateMainPipes = (map: google.maps.Map) => {
        ...
        if (existingMainPipes && existingSubMainPipes && existingLateralPipes) {
            try {
                ...
                if (mainPipesData.length > 0 && subMainPipesData.length > 0 && lateralPipesData.length > 0) {
        ...
        zones.forEach((zone, zoneIndex) => {
            ...
            pumps.forEach((pump) => {
                ...
                if (geometry?.spherical) {
                    ...
                    d = Math.hypot(zone.center.lat - pump.position.lat, zone.center.lng - pump.position.lng);
                }
        ...
        setTimeout(() => {
            try {
                const geometry = window.google?.maps?.geometry;
                const computePolylineLength = (path: Array<{ lat: number; lng: number }>): number => {
                ...
                let totalAreaSqm = 0;
                const zoneAreas: Array<{ zoneId: number; name: string; areaSqm: number; areaRai: number }> = [];
                if (geometry?.spherical) {
                    zones.forEach((zone) => {
                        let areaSqm = 0;
                        if (zone.coordinates && zone.coordinates.length >= 3) {
                            const latLngs = zone.coordinates.map((c) => new window.google.maps.LatLng(c.lat, c.lng));
                            areaSqm = geometry.spherical.computeArea(latLngs);
                        } else if (zone.bounds) {
                            const ne = new window.google.maps.LatLng(zone.bounds.north, zone.bounds.east);
                            const sw = new window.google.maps.LatLng(zone.bounds.south, zone.bounds.west);
                            const nw = new window.google.maps.LatLng(zone.bounds.north, zone.bounds.west);
                        ...
                        zoneAreas.push({ zoneId: zone.id, name: zone.name, areaSqm, areaRai: areaSqm / 1600 });
                    });
                }

                // 2) Plant count overall and per zone (rectangle boundaries fallback)
                const zonePlantCounts: Array<{ zoneId: number; name: string; plants: number }> = zones.map((zone) => ({ zoneId: zone.id, name: zone.name, plants: 0 }));
                plantPoints.forEach((pt) => {
                    const z = zones.find((zone) =>
                        zone.bounds && pt.position.lat >= zone.bounds.south && pt.position.lat <= zone.bounds.north && pt.position.lng >= zone.bounds.west && pt.position.lng <= zone.bounds.east
                    );
                    if (z) {
                ...
                if (newMainPipes.length > 0) {
                    const longestMainPipe = newMainPipes.reduce((longest, pipe) => {
                        ...
                        const longestLen = computePolylineLength([longest.fromPump, longest.toZoneCenter]);
                        return len > longestLen ? pipe : longest;
                    });
                    // Count sub-main pipes in the same zone as the longest main pipe
                    totalMainOutlets = (generatedSubMainPipes || []).filter(p => p.zoneId === longestMainPipe.zoneId).length;
                }
                
                // 2. Find the longest sub-main pipe across all zones
                let totalSubMainOutlets = 0;
                if ((generatedSubMainPipes || []).length > 0) {
                    const longestSubMainPipe = (generatedSubMainPipes || []).reduce((longest, pipe) => {
                    ...
                    totalSubMainOutlets = (generatedLateralPipes || []).filter(p => p.zoneId === longestSubMainPipe.zoneId).length;
                }
                
                // 3. Find the longest lateral pipe across all zones
                let totalLateralOutlets = 0;
                if ((generatedLateralPipes || []).length > 0) {
                    const longestLateralPipe = (generatedLateralPipes || []).reduce((longest, pipe) => {
                        const len = computePolylineLength(pipe.path);
                        const longestLen = computePolylineLength(longest.path);
                        return len > longestLen ? pipe : longest;
                    });
                    ...
                    totalLateralOutlets = plantPoints.filter(plant => 
                        Math.abs(plant.position.lng - lateralLng) < tolerance
                    ).length;
                }
                ...
                const zonePipeLengths = zones.map((zone) => {
                    ...
                    const mainLen = zoneMainPipes.reduce((sum, p) => sum + computePolylineLength([p.fromPump, p.toZoneCenter]), 0);
                    
                    // Sub-main pipe length and count
                    const zoneSubMainPipes = (generatedSubMainPipes || []).filter((p) => p.zoneId === zone.id);
                    const subLen = zoneSubMainPipes.reduce((sum, p) => sum + computePolylineLength(p.path), 0);
                    
                    // Lateral pipe length and count
                    const zoneLateralPipes = (generatedLateralPipes || []).filter((p) => p.zoneId === zone.id);
                    const latLen = zoneLateralPipes.reduce((sum, p) => sum + computePolylineLength(p.path), 0);
                    
                    // Get plants in this zone
                    const zonePlants = plantPoints.filter(point => 
                    ...
                    if (zoneLateralPipes.length > 0) {
                        ...
                        lateralOutlets = zonePlants.filter(plant => 
                            Math.abs(plant.position.lng - lateralLng) < tolerance
                        ).length;
                    }
    ...
    const findTreeColumns = (zoneBounds: { north: number; south: number; east: number; west: number }) => {
        ...
        const treesInZone = plantPoints.filter(point => {
            ...
            return lat >= zoneBounds.south && lat <= zoneBounds.north && 
                   lng >= zoneBounds.west && lng <= zoneBounds.east;
        });
        ...
        sortedTrees.forEach((tree, index) => {
            ...
            for (let i = 0; i < treeColumns.length; i++) {
                ...
                if (Math.abs(lng - columnLng) <= tolerance) {
                    ...
                    foundColumn = true;
                    console.log(`📍 Tree ${index + 1} added to existing column ${i + 1} (lng: ${lng.toFixed(6)})`);
                    break;
                }
            }
            ...
            if (!foundColumn) {
                ...
                console.log(`📍 Tree ${index + 1} created new column ${treeColumns.length} (lng: ${lng.toFixed(6)})`);
            }
        });
        ...
        treeColumns.forEach((column, index) => {
            ...
            console.log(`📏 Column ${index + 1}: ${column.length} trees, lng: ${column[0].lng.toFixed(6)}`);
        });
    ...
    const generateLateralPipes = (
        ...
        console.log('🔧 Generating lateral pipes perpendicular to sub-main pipes following tree columns');
        ...
        const currentSubMainPipes = generatedSubMainPipes && generatedSubMainPipes.length > 0 
            ? generatedSubMainPipes 
            : (subMainPipes.length > 0 ? subMainPipes : []);
        
        console.log(`📊 Using ${currentSubMainPipes.length} sub-main pipes for lateral pipe generation`);
        ...
        zones.forEach((zone, index) => {
            ...
            if (!zoneSubMainPipe || !zoneSubMainPipe.path || zoneSubMainPipe.path.length < 2) {
                console.log(`⚠️ No sub-main pipe found for zone ${zone.id}, skipping lateral pipes`);
                return;
            }
            ...
            if (treeColumns.length > 0) {
                ...
                treeColumns.forEach((column, columnIndex) => {
                    ...
                    // Add to map
                    polyline.setMap(map);
                    console.log(`✅ Successfully created lateral pipe ${columnIndex + 1} for zone ${zone.id}`);
    ...
    const getButtonState = (stepIndex: number) => {
        console.log(`getButtonState(${stepIndex}): currentStep=${currentStep}, completedSteps=[${completedSteps.join(',')}]`);
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-slate-700 via-slate-600 to-slate-700">
            ...
            <div className="mx-auto max-w-5xl px-4 py-4 md:px-6 md:py-6">
                ...
                <div className="mb-4 h-[420px] rounded-lg border border-slate-600 bg-slate-700/40 overflow-hidden relative">
                    {/* Search Component */}
                    <div className="absolute top-1 left-32 z-10 w-48 sm:w-56 md:w-80 lg:w-96">
                        <div className="relative">
                            <input
                                type="text"
                                value={searchValue}
                            ...
                            {showSearchResults && searchResults.length > 0 && (
                                <div className="absolute top-full left-0 right-0 mt-1 max-h-48 md:max-h-60 overflow-y-auto rounded-lg border border-slate-300 bg-white shadow-lg">
                                    {searchResults.map((result) => (
                                        <div
                                            key={result.place_id}
                                            onClick={() => handleSearchResultClick(result.place_id)}
                                            className="cursor-pointer px-3 py-2 text-xs md:px-4 md:py-2 md:text-sm text-gray-700 hover:bg-blue-50"
                                        >
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="mx-4 w-full max-w-md rounded-lg bg-slate-800 p-6 shadow-xl">
                        <div className="mb-4 text-center">
                            <h3 className="text-lg font-semibold text-white">Determine number of zones</h3>
                        </div>
                        ...
                        <div className="mb-6">
                            ...
                            <div className="mb-4">
                                <p className="text-sm text-slate-400">
                                    Each zone will have a unique color and will be divided horizontally across your area.
                                </p>
                            </div>
                            
                            {/* Color preview */}
                            <div className="mb-4">
                                <p className="text-sm font-medium text-slate-300 mb-2">Zone colors preview:</p>
                        ...
                        <div className="flex gap-3">
                            <button
                                ...
                                className="flex-1 rounded-lg bg-slate-600 px-4 py-3 text-white hover:bg-slate-500 transition-colors"
                            >
                                Cancel
                            </button>
                            <button
                                onClick={handleGenerateZones}
                                className="flex-1 rounded-lg bg-green-600 px-4 py-3 text-white hover:bg-green-700 transition-colors"
                            >
                                Generate
                            </button>
                        </div>
